...
const uint8_t statled1 = 0;  //This is the normal status LED
const uint8_t statled2 = 1;  //This is the SPI LED, indicating SD traffic
...
void command_shell(void)
{
  ...
  while(true)
  {
    ...
    else if(strncmp(command_arg, "rm", 2) == 0)
    {
      ...
      if ((count_cmd_args() == 3) && (strncmp(command_arg, "-rf", 3) == 0))
      {
    ...
    else if(strncmp(command_arg, "write", 5) == 0)
    {
      ...
      while(1) {
        ...
        if ((feedback_mode & EMBEDDED_END_MARKER) > 0)
          NewSerial.print((char)0x1A); // Ctrl+Z ends the data and marks the start of result
#endif
        NewSerial.print("<"); //give a different prompt
    ...
    else if(strcmp(command_arg, "pwd") == 0)
    {
      ...
      for (uint8_t i = 0; i < tmp_var; i++)
      {
        ...
        if (i < tmp_var-1) NewSerial.print("\\");
      }
      NewSerial.println("");
#ifdef INCLUDE_SIMPLE_EMBEDDED
      command_succedded = 1;
    ...
    else if(strncmp(command_arg, "eem", 3) == 0)
    {
      //Argument 2: <on>|<off>
...
//ERROR
     ...
     else if(strncmp(command_arg, F("efcount"), 7) == 0)
     {
     ...
     else if(strncmp(command_arg, F("efinfo"), 6) == 0)
     {
